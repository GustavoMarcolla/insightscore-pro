...
export default function Dashboard() {
  ...
  return <div className="space-y-6">
      ...
      <Card className="border-2 border-primary/20 bg-gradient-to-br from-card to-primary/5">
        ...
        <CardContent>
          {isLoading ? <Skeleton className="h-64 w-full" /> : <ResponsiveContainer width="100%" height={280}>
              <AreaChart data={monthlyScores} margin={{
            top: 10,
            right: 30,
            left: 0,
            bottom: 0
              ...
              borderRadius: "8px"
                ...
                <ReferenceLine y={80} stroke="hsl(var(--success))" strokeDasharray="5 5" label={{
              value: "Meta 80%",
              position: "right",
              fontSize: 10,
              fill: "hsl(var(--success))"
            }} />
                <ReferenceLine y={70} stroke="hsl(var(--warning))" strokeDasharray="5 5" label={{
              value: "Mínimo 70%",
              position: "right",
              fontSize: 10,
              fill: "hsl(var(--warning))"
            }} />
                <Area type="monotone" dataKey="avg_score" stroke="hsl(var(--primary))" strokeWidth={2} fill="url(#scoreGradient)" dot={{
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="card-section">
          ...
          <div className="space-y-3">
            ...
            length: 5
          }).map((_, i) => <Skeleton key={i} className="h-10 w-full" />) : topSuppliers.length === 0 ? <p className="text-sm text-muted-foreground text-center py-4">Nenhum fornecedor avaliado ainda</p> : topSuppliers.map((supplier, index) => <div key={supplier.id} className="flex items-center justify-between rounded-md bg-secondary/50 px-3 py-2 animate-slide-in" style={{
        ...
        <div className="card-section lg:col-span-2">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {isLoading ? Array.from({
            length: 4
          }).map((_, i) => <Skeleton key={i} className="h-20 w-full" />) : lowScoreCriteria.length === 0 ? <p className="text-sm text-muted-foreground text-center py-4 col-span-full">
                Nenhuma avaliação de critérios ainda
              </p> : lowScoreCriteria.map((criteria, index) => <div key={criteria.id} className="rounded-md border border-border p-4 text-center animate-fade-in" style={{
